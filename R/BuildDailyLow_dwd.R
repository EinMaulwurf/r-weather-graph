...
# --- 5. Create the Main Plot (Min Temperature Focus) ---
...
min.graph <- daily.summary.stats %>%
  ...
  # This year's line
  geom_line(
    data = this.year %>% filter(!is.na(TMIN)), # Ensure no NAs break the line
    aes(y = TMIN),
    linewidth = 0.8,
    color = "black"
  ...
  # Labels and Caption
  labs(
    title = paste("Tagestiefsttemperatur in", "Frankfurt am Main"),
    subtitle = paste0(
      "Linie zeigt Tagestiefsttemperatur für ",
      year.to.plot,
      ". ",
...
# Save the final plot
ggsave("graphs/DailyLowTemp_dwd.png", plot = min.graph2, width = 8, height = 4)
